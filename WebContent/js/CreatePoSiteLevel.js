...
function deleteRow(btn, currentRow) {
	debugger;	
	alert("Do you want to delete this Charges row.");
	document.getElementById("hiddenSaveBtnId").value = "";
...
function deleteproductRow(btn, currentRow) {
	...
	alert("Do you want to delete this Product row.");
...
function validateProductAvailability(unitsFld) {
	...
	if(measurementId != "" && measurementId != '' && measurementId != null) {
		...
		productAvailability(childPro,groupId,siteId,currentRowNum);
	}
	else {
...
function productAvailability(childProdId,groupId,siteId,currentRowNum) {debugger;
	...
	//alert(url);
	var url="gettingPOBoqQuantityAjax.spring?childProductId="+childProdId+"&groupId="+groupId+"&siteId="+siteId;
	var	request = getAjaxObject();
	...
	try {
		request.onreadystatechange = function() {
			...
			if(request.readyState == 4 && request.status == 200) {
	    		...
	    		$("#avalBOQQty"+currentRowNum).val(resp.split("_")[0]);
	    		$("#BOQQty"+currentRowNum).val(resp.split("_")[1]);
...
function validateUnitsAndAvailability(currentRowNum) {
	...
	if(count==0){
		...
		var currentGroupId1=$("#groupId"+currentRowNum).val();
		var matchedEnteredQty=0;
		...
		debugger;
		matchedEnteredQty=matchedEnteredQty.toFixed(3);
		if(parseFloat(matchedEnteredQty)>parseFloat(totalQty) && currentGroupId1!='0'){
