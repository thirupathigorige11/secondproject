...
(function( $ ) {
  ...
  _create: function() {
    ...
    this.input = $( "<input>" )
      ...
      //  alert("str1 "+str1);
        ...
        else if(str1 == "subProduct" || str1 == "SubProduct") {
        //	alert("SubProduct ");
        ...
        else if(str1 == "childProduct" || str1 == "ChildProduct") {
        	prodId = ui.item.option.value;
            prodName = ui.item.value;        
	  ...
	  $("#receivedDate").datepicker({
		  dateFormat: 'dd-mm-y',
			  ...
			  onSelect :function(dateText){
		       ...
		       var DCnumber= $("#DCNumberId").val();
		       var DCvend= $("#vendorIdId").val();
		       var DCdate= $("#receivedDate").val();
		       var url ="getDCCount.spring?DCNumber="+DCnumber+"&vendorname="+DCvend+"&receiveDate="+DCdate;
		    	 var vName=""
		    	   $.ajax({
		    		   url :url,
		    		   type:"post",
 ...
 function validatePoNum(){//alert();
  ...
  function setVendorData(vName) {
	...
	var url = "loadAndSetVendorInformation.spring?vendName="+vName;
...
function setTransportData(vName) {
...
function appendRow() {
    ...
    //for first row delete button showing 
		if(tbllength==2){
...
function loadSubProds(prodId, rowNum) {
	...
	var url="dcformSubProducts.spring?mainProductId="+prodId;
...
function loadSubSubProducts(subProdId, rowNum) {
	...
	var url="dcformChildProducts.spring?subProductId="+subProdId;
...
function loadUnits(prodId, rowNum) {
	...
	var url = "listdcformChildProducts.spring?productId="+prodId;
   ...
   if(transportDetails == "" || transportDetails == '' || transportDetails == null) {
	...
	for(var x=0; x<snoId; x++) {
		...
		var charges = (individualBasicAmount * convenceCharges) / rsltTtlBscAmnt;
			...
			charges=parseFloat(convenceCharges);
		}
...
function calculateTotalBasicAmount() {
	...
	//alert("Total Basic Amount = "+totalBasicAmount);
...
function getHighestTax() {
	...
	for(var x=0; x<snoId; x++) {	
		var val = spltdData[x];
		var selectedTax = document.getElementById("GSTTax"+val);
...
function validateRowData() {	
	...
	var DCNum = document.getElementById("DCNumberId").value;
	//alert(invoiceNum);
	
	if(DCNum == "" || DCNum == null || DCNum == '') {
		alert("Please enter DC Number.");
		document.getElementById("DCNumberId").focus();
		return false;
	}				
	  
	var DCcDate = document.getElementById("DCDate").value;
	//alert(invoiceDate);
	
	if(DCcDate == "" || DCcDate == null || DCcDate == '') {
    ...
    var DCstate = document.getElementById("state").value;
    if(DCstate == "" || DCstate == '' || DCstate == null) {
		alert("Please Choose State.");
		document.getElementById("state").focus();
		return false;
	}
    
    var typeOfPurchase = document.getElementById("state1").value;
    //var DCstate = document.getElementById("state").value;
    if(typeOfPurchase == "" || typeOfPurchase == '' || typeOfPurchase == null) {
		alert("Please Choose Recevied From.");
		document.getElementById("state1").focus();
		return false;
	}
    
    var receiveFrom = $("poNumber").val();
   // var indentNumber = $("indentNumber").val();// indentNumber
   


    document.getElementById("indentNumber").value;
    
    if(typeOfPurchase == 'PO'){
   // alert("po");
    
        if((receiveFrom == "" || receiveFrom == '' || receiveFrom == null ) ) {
    	...
    	//alert("indent");
    	//alert(indentNumber);
    	 if((indentNumber == "" || indentNumber == '' || indentNumber == null ) ) {
    	...
    	for (var i in elementList) {
    		...
    		if (elementList[i].id != "") {
	    	  ...
	    	  if(elementList[i].id == prodId) {
	    		  ...
	    		  var PriceAmt = document.getElementById(priceId).value;
	    		  if(PriceAmt == "" || PriceAmt == null) {
...
function validateProductAvailability(unitsFld) {
	...
	if(measurementId != "" && measurementId != '' && measurementId != null) {
		...
		//alert("receivedDate "+receivedDate);
...
var expireDateId = "expireDateId"+curRow;
	...
	document.getElementById("doDCFormId").action = "submitDCForm.spring";
	document.getElementById("doDCFormId").method = "POST";
	document.getElementById("doDCFormId").submit();
}
...
var expireDateId = "expireDateId"+curRow;
	...
	var otherChrgs = document.getElementById("ConveyanceAmount").value;
	
	if(otherChrgs == "" || otherChrgs == '' || otherChrgs == null) {
		alert("Please enter Other Chargers.");
		document.getElementById("ConveyanceAmount").focus();
		return;
	}
