...
function validateRowData() {	
	...
	if(invoiceNum.length<=4){
		...
		$('#InvoiceNumberId').setCursorPosition(4);
		document.getElementById("InvoiceNumberId").focus();
		alert("Please enter Invoice Number.");
		return false;
	}
...
function productAvailability(mainProdId, subProdId, childProdId, measurementId, currentRowNum,requestedDate) {
	var site_id=$("#vendorIdId").val();debugger;
	var url = "getProductAvailability.spring?prodId="+mainProdId+"&subProductId="+subProdId+"&childProdId="+childProdId+"&measurementId="+measurementId+"&requesteddate="+requestedDate+"&isMaterialAdjustment=yes&site_id="+site_id;
	//alert(url);
	    			...
	    			return false;
	    	    	 ...
	    	    	 if(typeOfPurchase == 'marketPurchase' && (indentNumber == "" || indentNumber == '' || indentNumber == null ) ) {
		    			...
		    			return false;
	    	    		  ...
	    	    		  if((typeOfPurchase == 'localPurchase' && finalAmntDiv>=10000) && (indentNumber == "" || indentNumber == '' || indentNumber == null )) {
