...
function appendChargesRow() {debugger;
	var calQuantityamount=CalucalteQuantityamount();
	if(calQuantityamount==false){
		return false;
	}
	var validateChargesTable=validateChargesRow();
    if(validateChargesTable==false){
    	return false;
    }
	...
	if(pressedKey.keyCode == 13 || pressedKey.keyCode == undefined || pressedKey.keyCode == "undefined") {
	    ...
	    if(hiddenSaveBtn == "" || hiddenSaveBtn == '' || hiddenSaveBtn == null) {
		    ...
		    $(row).attr("id", "chargesrow"+rowChargesNum);
		    $(row).attr("class", "conveyancetrClass");
	...
	$('#Conveyance'+btn).change(function() {
		if ($(this).val() == "999$None"){
		  // Updating text input based on selected value
		  $(".ConveyanceAmount1").css("readonly","true");
		  $(".GSTTax1").css("readonly","true");
		  $(".noneClass").val(0);		  
		  $(".GSTClass").val( '1$0%');
...
function maincalculatequantitybase(el,rowId){debugger;

var Quantity = $("#stQantity"+rowId).val();
if(isNaN(Quantity)){
...
if(isNaN(Disocuntval)){
	alert("Disocunt is not a number");
	$("#Discount"+rowId).val("");
	$("#Discount"+rowId).focus();
	return false;
}
if(Disocuntval>100){
	alert("Disocunt should be less than or equal to 100.");
	$("#Discount"+rowId).val("");
	$("#Discount"+rowId).focus();
	return false;
}
...
//taking  the tax
//var e1 = document.getElementById("taxAmount"+rowId);
var selectetdtax =$("#taxAmount"+rowId).val();
//alert(e);
...
// Calculating the tax 
var taxAmntVal = (DiscountamtAfrTax/100) * selectedTax1;
...
if(calQuantityamount==false){
	...
	var valChargesTable=validateChargesRow();
...
//code done by ashok for number checking
function findNumberorNot(){
	var classLength = $(".tblrowClass").length;
    for (var i=0;i<=classLength;i++){
    	var rowId = i+1;
	var Quantity = $("#stQantity"+rowId).val();
	debugger;
	var Quant = $.isNumeric(Quantity);
	if(Quant == false){
		alert("Please enter valid quantity");
		$("#stQantity"+rowId).val("");
		$("#stQantity"+rowId).focus();
...
//code for number checking
//code for validation
function CalucalteQuantityamount(){
	debugger;
	var error=true;
    var classLength = $(".tblrowClass").length;
    for (var i=0;i<classLength;i++){
    	var id = i+1;
	var Qunatity = $("#stQantity"+id).val();
