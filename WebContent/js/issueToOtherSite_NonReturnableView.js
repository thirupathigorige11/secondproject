...
$(function() {
	$("#expiryDateId1").datepicker({dateFormat: 'dd-M-y',  minDate:0});
});
$(function() {
	$("#InvoiceDateId").datepicker({dateFormat: 'dd-M-y',maxDate: new Date()});
});

$(function() {
	$("#receivedDate").datepicker({dateFormat: 'dd-M-y',maxDate: new Date()});
});
...
//delete charges row written by thirupathi
...
function calculateOtherCharges() {debugger;	
   ...
   var valchargeStatus = validatechargesrow();
			if(valchargeStatus == false) {
	    	return false;
		}
	...
	for(var x=0; x<resetSnoId; x++) {
		var totalAMount= document.getElementById("totalAmount"+resetSnoId).value;
	 ...
	 document.getElementById("totalAmount"+resetval).value = "";
		...
		for(var i=0; i<=chargeSnoId-1; i++) {
			...
			for(var x=0; x<snoId; x++) {	for(var x=0; x<snoId; x++) {
				...
				var amntAfterTax = parseFloat(individualBasicAmount);
...
//validating charges row written by thirupathi

function validatechargesrow(){
	debugger;
	 var tablelength=$("#doInventoryChargesTableId > tbody > tr").length;
	 for(var i=1;i<=tablelength;i++){
		 var Conveyance=$("#Conveyance"+i).val();
		 var ConveyanceAmount=$("#ConveyanceAmount"+i).val();
		 var GSTTax=$("#GSTTax"+i).val();
		 if(Conveyance=="" || Conveyance=="null" || Conveyance==undefined){
			alert("Please select Conveyance.");
			$("#Conveyance"+i).focus();
			return false;
		 }
		 if(ConveyanceAmount=="" || ConveyanceAmount=="null" || ConveyanceAmount==undefined){
				alert("Please enter Conveyance Amount.");
				$("#ConveyanceAmount"+i).focus();
				return false;
			 }
		 if(GSTTax=="" || GSTTax=="null" || GSTTax==undefined){
	...
	/*************************************** check the boq quantity for user click on submit button start *******************************/

function validateQtyOnSubmit(){debugger;
	...
	$(".productTableRow").each(function(){
		...
		//if($("#idDeletedOrNot"+currentId).val()!="D"){
			...
			$(".productTableRow").each(function(){
				...
				if(currentGroupId==currentGroupId1){
					matchedEnteredQty+=parseFloat($("#quantity"+currentId1).val()==""?0:$("#quantity"+currentId1).val());
				}			
			});
			avalBOQQty=parseFloat(avalBOQQty).toFixed(3);
			if(parseFloat(matchedEnteredQty)>parseFloat(avalBOQQty)){
				alert("You can not initiate Child Product"+$("#ChildProduct"+currentId).val().split("$")[1]+" more than "+BOQQTY+" "+$("#UnitsOfMeasurement"+currentId).val().split("$")[1]+". As it is exceeding BOQ Quantity.");
				//$("#quantity"+currentId).val('');
				//$("#quantity"+currentId).focus();		
				return error=false;
			}
		//}			
	});
