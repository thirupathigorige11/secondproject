...
/********************** Calculate Discount Amount**************************/
	...
	function calculateDiscountAmount(strSerialNumber){
		...
		if(discountval > 100){
			...
			/* document.getElementById(discountnum).focus(); */
			return;
		}
...
/*function calcuLateFinalAmount() {
	...
	function calculatequantitybase(rowId){debugger;
		...
		document.getElementById("TotalAmountId"+ rowId).value = amontAfterTaxVal;
	...
	function calcuLateFinalAmount() {
	    ...
	    document.getElementById("ttlAmntForIncentEntryId").value = parseFloat(tempAmnt).toFixed(2);
	}
...
function calculateGSTTaxAmount(rowNum) {
	...
	var selectedTax = e.options[e.selectedIndex].value;
	//alert(selectedTax);
	var ConveyanceAmount=$("#ConveyanceAmount"+ rowNum).val()==""?0:$("#ConveyanceAmount"+ rowNum).val();
	if (isNaN(ConveyanceAmount)) {
		debugger;
		alert("Please enter valid data.");
		$("#ConveyanceAmount"+rowNum).val("0");
...
/* var subProd= $("#subProduct"+1).val();*/
// alert(subProd);
	/*var childProdct= $("#childProductt"+1).val();*/
	//alert(childProdct);
/*	var Uom= $("#unitsOfMeasurement"+1).val();
$.each($('.btn-visibilty'+1),function(index,row){*/
/*	$(this).closest('td').find('input').removeAttr('disabled');
	...
	/*productId = $('#combobox'+1).val();*/
	//alert(productId);
	
	
	/*SubproductId = $('#SubProduct'+1).val();*/
	//alert(SubproductId);
	//loadSubSubProducts(SubproductId,rowId);
	
/*	ChildproductId = $('#ChildProduct'+1).val();*/
	//alert(ChildproductId);
	//loadUnits(ChildproductId,rowId);
/*});*/

/*loadSubProds(productId,1, 'edit');
*/
//alert(subProd);
...
function calculateChangedAmount(rowId){debugger;



var Quantity=$("#stQantity"+ rowId).val()==""?0:$("#stQantity"+ rowId).val();
var price= $("#price"+ rowId).val()==""?0:$("#price"+ rowId).val();
var ConveyanceAmount=$("#ConveyanceAmount"+ rowId).val()==""?0:$("#ConveyanceAmount"+ rowId).val();
if (isNaN(Quantity)) {
	debugger;
	alert("Please enter valid data.");
...
/*if (isNaN(ConveyanceAmount)) {
	debugger;
	alert("Please enter valid data.");
	$("#ConveyanceAmount"+rowId).val("0");
	$("#ConveyanceAmount"+rowId).focus();
	
	return false;
}*/
//Calculating the basic amount
var BasicAmonmt=(parseFloat(Quantity)*parseFloat(price));
BasicAmonmt=BasicAmonmt.toFixed(2);
/*if (isNaN(BasicAmonmt)) {
	debugger;
	alert("Please enter only digits 3.");
	$("#BasicAmountId"+rowId).val("0");
	$("#BasicAmountId"+rowId).focus();
...
// Calculating the discount
var Disocuntval=document.getElementById("Discount"+ rowId).value;
if (isNaN(Disocuntval)) {
	debugger;
	alert("Please enter valid data.");
	$("#Discount"+rowId).val("0");
	$("#Discount"+rowId).focus();
...
var DiscountamtAfrTax=(parseFloat(BasicAmonmt - ( BasicAmonmt*Disocuntval/100 )));
DiscountamtAfrTax=DiscountamtAfrTax.toFixed(2);
// Appending the discount amount
document.getElementById("amtAfterDiscount"+ rowId).value = DiscountamtAfrTax;
...
// Calculating the tax 
var taxAmntVal = ((DiscountamtAfrTax/100) * selectedTaxval);
taxAmntVal=taxAmntVal.toFixed(2);
...
//
var amontAfterTaxVal= (parseFloat(DiscountamtAfrTax) + parseFloat(taxAmntVal));
amontAfterTaxVal=amontAfterTaxVal.toFixed(2);
document.getElementById("TaxAftertotalAmount"+ rowId).value = amontAfterTaxVal;
...
function calculateOtherCharges() {debugger;
	 ...
	 var chargeSno = getChargeTotalSNOS();
		...
		for(var y=0; y<chargeSnoId; y++) {
			...
			var chargeVal = snoSpltdData[y];
			 convenceCharges = parseFloat($('#ConveyanceAmount'+chargeVal+'').val().trim()==""?0:$('#ConveyanceAmount'+chargeVal+'').val());
	...
	for(var x=0; x<snoId; x++) {
		...
		var existVal;
		if (snoSpltdData[y] > 1) {
			existVal = $("#OtherOrTransportChargesId"+val).val();
...
function calculateTotalBasicAmount() {debugger;
	...
	for(var x=0; x<snoId; x++) {
		...
		var val = spltdData[x];
		var basicAmnt = $("#amtAfterDiscount"+val).val()==""?0:$("#amtAfterDiscount"+val).val();
...
function validateRowData(strSerialNumber) {
	...
	var TaxAmnt = document.getElementById("taxAmount"+strSerialNumber).value;
	//alert(invoiceDate);
	
	if(TaxAmnt == "" || TaxAmnt == null || TaxAmnt == '') {
		alert("Please enter the Tax.");
		document.getElementById("TaxAmnt").focus();
		/*s*/
	}	
/*	var uom = document.getElementById("UnitsOfMeasurementId"+strSerialNumber).value;
	//alert(Measurement);
...
//validating product table rows
function validateProducttable(){
	debugger;
	var error=true;
	var TableLength=$("#tblnotification").find("tbody").find("tr").length;
	for(var i=0;i<TableLength;i++){
		var stQantity=$("#stQantity"+(i+1)).val().trim();
		if(stQantity==""){
			alert("Please enter quantity");
			$("#stQantity"+(i+1)).focus();
			return error=false;
		}
		var price=$("#price"+(i+1)).val().trim();
		if(price==""){
			alert("Please enter price");
			$("#price"+(i+1)).focus();
			return error=false;
		}
		var Discount=$("#Discount"+(i+1)).val().trim();
		if(Discount==""){
			alert("Please enter discount");
			$("#Discount"+(i+1)).focus();
			return error=false;
		}
		var taxAmount=$("#taxAmount"+(i+1)).val().trim();
		if(taxAmount==""){
			alert("Please select taxAmount");
			$("#taxAmount"+(i+1)).focus();
			return error=false;
		}
	}
...
function validateChargesRow(){
	...
	$(".chargesrowCls").each(function(){
		...
		if(ConveyanceAmount==""){
			alert("Please select Conveyance Amount");
