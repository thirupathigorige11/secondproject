	     ...
	     if(varServiceType == 'add'){
	    	 ...
	    	 //var AllProducts=$("#AllProducts").val();
	    		$.ajax({
					type: 'POST',
					//dataType: 'json',
					url: 'viewaddedAllproduct.spring?AllProducts=true',
					...
					success : function(data) {
						...
						//  alert(dataLen);
						  $('#records_table').empty();
						  trHTML += ' <thead class="cal-thead-inwards"><tr><th>S No</th><th>Products</th><th>Product Type</th></tr></thead>';
						  trHTML += '<tbody class="tbl-fixedheader-tbody">';
						  debugger;
			              for (i=0; i<dataLen; i++) {
			            	  trHTML += '<tr><td>' + data[i].serialno + '</td><td>' + data[i].productName + '</td><td>' + data[i].productType + '</td></tr>';
			              }	
			              trHTML += '</tbody>';
...
function formValidator(){debugger;
	...
	if(serviceType == "add"){
		...
		var dept_Type = document.getElementById('Product_Type').value; 
		 if(add =='' || add == null){
			 ...
			 $("#product").focus();
				return false;
		 }else if(dept_Type =='' || dept_Type == null){
		...
		var dept_Type = document.getElementById('Delete_Product_Type').value;
	...
	//var Delete_product= document.getElementById('Delete_Product_Type').value;
	//debugger;
	
	/* if(add=="" || add == null && Delete_product==null || Delete_product==""){
		 alert("Please select prodcut type.");
		 $("#Delete_Product_Type").focus();
		 return false;
//	 }
	}
					...
					<div class="right_col" role="main">
						...
						<div align="center">
						  <div class="addvendorfrm">
						    ...
						    <form name="myForm" action="savestoreandMarketingproduct.spring"   onsubmit="return formValidator()"  method="post" class="form-horizontal border-inwards-box">
										...
										<div class="input_div">
											...
											<%-- <c:if test="${AllProducts}"> --%>
												<div class="form-group">
										      		...
										      		<div class="col-md-6">
										         		 <input  type="text"  name="product" id="product" onkeyup="populateSite(this)" class="form-control pordtxtname">
										     		 </div>
											</div>
											<%-- </c:if> --%>
								 		</div>
...
<div class="ui-widget">
   <div class="form-group"">
    ...
    <%-- <c:if test="${Delete}"> --%>
									<div class="form-group"">
										<label label class="commlbl col-md-6">Product Type : </label>
										<div class="col-sm-6">
									...
									<%-- </c:if> --%>
									    ...
									    <input type="submit" value="Submit" id="submitFormData" class="btn btn-warning" name="Submit">
									</div>
	...
	/* ****************************************for auto complete user choos the already avail or not***************************************** */		

function populateSite(id) {debugger;
  var Product=$("#product").val();
  var Product_Type=$("#Product_Type").val();
  var url = "ViewProductForAutoComplete.spring?Product="+Product+"&Product_Type="+Product_Type;
