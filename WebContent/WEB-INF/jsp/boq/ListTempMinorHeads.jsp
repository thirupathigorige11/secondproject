 ...
 <div class="col-md-12">
	...
	<!-- TABLE -->
				...
				<div class="table-responsive" >
					<table id="example"	class="table table-bordered table-new display table-fixed-header" cellspacing="0" style="width:100%;">
						<thead>
							<tr>
								<th>Major Head</th>
								<th style="width:200px;">Minor Head </th> 
						...
						<tbody>
							<c:forEach items="${minorHeadsDetails}" var="element">
							<c:forEach var="boqMainList" items="${requestScope['boqData']}">
								<tr>
									<td>${element.strMajorHeadDesc}</td>
									<td><a class="anchor-class" href="getTempBOQDetails.spring?tempBOQNo=${boqMainList.intTempBOQNo}&siteId=${boqMainList.strSiteId}&boqType=${boqMainList.boqType}&combobox_Product=${element.strMajorHeadId}&combobox_SubProduct=${element.strMinorHeadId}&viewTempBoq=${viewTempBoq}&pageHighlightURL=${pageHighlightURL}"  title="Click on the product name to see the details">${element.strMinorHeadDesc}</a>  </td>
									<td title="${element.strMinorHeadMaterialTotalAmount}">${element.strMinorHeadMaterialTotalAmount}</td>
									<td title="${element.strMinorHeadLaborTotalAmount}">${element.strMinorHeadLaborTotalAmount}</td>
									<td title="${element.strMinorHeadTotalAmount}">${element.strMinorHeadTotalAmount}</td>
								</tr>
								</c:forEach>
							</c:forEach>	
				       </tbody>
