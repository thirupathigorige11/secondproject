 ...
 <div class="col-md-12">
		...
		<thead >
				<tr>
    				...
    				<c:if test="${recordType == 'MATERIAL'}">
    				<th  width=213px;class="w-70">Material Group</th>
    				<th  width=213px;class="w-70">UOM</th>
    				<th  width=213px;class="w-70">PerUnit Quantity</th>
    				<th  width=213px;class="w-70">PerUnit Amount</th>
    				<th  width=213px;class="w-70">Material Action</th>
    				</c:if>
  			...
  			<tr>
  					...
  					<c:if test="${recordType == 'MATERIAL'}">
    				<td>${boqData.materialGroupName }</td>
    				<td>${boqData.materialGroupUOM }</td>
    				<td>${boqData.materialPerUnitQuantity }</td>
    				<td>${boqData.materialPerUnitAmount }</td>
    				<td>${boqData.materialAction }</td>
    				</c:if>
...
<!-- 			 <div class="modal fade" id="myModal" role="dialog">
			...
			<c:forEach var="boqMainList" items="${requestScope['boqData']}">
				<a href="getModificationDetails.spring?tempBOQNo=${boqMainList.intTempBOQNo}&siteId=${boqMainList.strSiteId}&boqType=REVISED&recordType=${recordType}&majorHeadId=${majorHeadId}&viewTempBoq=${viewTempBoq}&pageHighlightURL=${pageHighlightURL}" class="btn btn-warning">Modification Details</a>
			</c:forEach>
			<br></br>
			<%-- <div class="row">
				<div class="form-group">
