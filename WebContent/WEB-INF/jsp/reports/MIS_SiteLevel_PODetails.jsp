			...
			<div class="right_col" role="main">
				<div>
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">MIS Site PO Details</li>
					</ol>
				</div>
					...
					<div class="container">
						...
						<form class="form-horizontal">
						 <table id="mistableFixedheader" class="table table-bordered display table-fixed-header">
                            ...
                            <tbody>
                            <%
                            List<ProductDetails> poDetails = (List<ProductDetails>) request.getAttribute("siteLevelPoDetails");
                            int i=1;
                              ...
                              <tr><%--  <td><%=i%></td> --%>
                                <%   
                                    ...
                                    %>
										...
										<td>${element.indentNo}</td>
										<td>${element.indentDate}</td>
										<td>${element.quatations}</td>
										<td>${element.strPONumber}</td>
										<td>${element.poDate}</td>
										<td>${element.purchase_po_Req_Date}</td>
										<td>${element.vendorName}</td>
										<td>${element.poTotal}</td>
										<td>${element.deliveryDate}</td>
										<td>${element.materialDesc}</td>

										<td>${element.invoiceNumber}</td>
										<td>${element.receiveDate}</td>
										<td class="tatlAmt">${element.invoiceAmount}</td>
										<td>${element.grnDate}</td>
							...
							<!-- </tbody> -->
							<tr class="info">
								<td colspan="10" class="text-right subTtl">SUB
									TOTAL:</td>
