...
<div class="">
	...
	<form:form  id="indentIssueFormId" action="ViewThreeMonthProductPrice.spring" class="form-horizontal border-inwards-box">
			...
			<%-- <div class="col-md-4">
				 <div class="form-group">
					...
					<div class="col-md-6"> <input id="childProdName" name="childProdName" autocomplete="off" class="form-control" size="20" value="<c:out value='${childProdName}'/>">
						<button  type="button" class="btn btn-default clearbtn" onclick="clearText()"><i class="fa fa-close"></i></button>
			...
			<div class="col-md-4">
				<div class="form-group">
				<label class="col-md-6 control-label">Child Product Name :</label>
				<div class="col-md-6"> <input id="childProdName" name="childProdName" autocomplete="off" class="form-control" size="20" value="<c:out value='${childProdName}'/>" title="${childProdName}">
				<button type="button" class="btn clearbtn" title="Clear Search" onclick="clearText()">x</button>
				<input type="hidden" id="childProdID">
				<input type="hidden" id="tempchildProdID">
				</div>
				</div>
						...
						<tr>
							...
							<c:choose>
								...
								<c:when test="${not empty product.invoice_number}">
									<td>
										<c:if test="${product.indentType eq 'INU'}">
											INV_${product.invoice_number}
										</c:if>
										<c:if test="${product.indentType ne 'INU'}">
											<a  target="_blank" href="getGrnDetails.spring?invoiceNumber=${product.invoice_number}&vendorId=${product.vendor_id}&invoiceDate=${product.indent_recive_date}&indentEntryId=${product.indent_entry_id}&type=invoicePriceMaster&siteId=${product.site_id}&showHomebtn=false&url=${url}&childProdName=${childProdName}&monthYear=${monthYear}&PriceMaster=${priceMaster}" style="text-decoration: underline;color: blue;" onclick="loader();">INV_${product.invoice_number} </a> 
										</c:if>
									</td>
								</c:when>
...
$(window).load(function () {
  $(".overlay_ims").hide();	
	...
	//Animate loader off screen
		/* $(".overlay_ims").fadeOut("slow");
		$(".loader-ims").fadeOut("slow"); */
});
