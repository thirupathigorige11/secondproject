...
String vendorId=null;
    ...
    String receiverContactPerTwo=null;
...
<input type="hidden" name="deliveryDate" id="deliveryDate" value="${strDeliveryDate}">
<input type="hidden" name="strVendorName" id="strVendorName" value="${vendorName}">
...
<%   
           	...
           	%>
	            <tr class="n-b" style="border: 2px solid black;">	            	
	             ...
	             <%--    <td style="font-size: 16px;font-weight: bold;"><%= splData[1] %></td> --%>
	                <td style="font-size: 16px;font-weight: bold;width: 160px;border: 2px solid black;"><a href="javascript:void(0)" onclick="loadPriceMasterData('<%=splData[27]%>','<%= splData[2] %>')">   <%= splData[2] %> </a> </td>
              ...
              //  List<ProductDetails> getPoDetails = new ArrayList<ProductDetails>();
           	...
           	%>
	            <tr style="border: 2px solid black;">	            	
	              ...
	              <%--   <td style="font-size: 16px;font-weight: bold;"><%= splData[1] %></td> --%>
	                <td style="font-size: 14px;font-weight: bold;width: 160px;border: 2px solid black;;"><a href="javascript:void(0)" onclick="loadPriceMasterData('<%=splData[27]%>','<%= splData[2] %>')">   <%= splData[2] %> </a> </td>
 ...
 %>
     ...
     %>
     <p style="margin-left:20px;"><strong>Comments:</strong></p>
     <p style="margin-left:20px;"><c:out value="${PoLevelComments}"></c:out></p>
     <%} %>
...
<div id="modal-canceltemPo" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title text-center">Reject Temp PO Remarks</h4>
      </div>
      <div class="modal-body" style="overflow:hidden;">
       <div class="col-md-12">
...
<div id="myModal-pdf4" class="modal fade" role="dialog">
 ...
 <button class="btn btn-warning print-btn" onclick="createModelPOPUP()"  id="approve_temp_po" style=" height: 35px; background: #b3b5f;margin-bottom:10px;">Approve</button>
 <button  onclick="createModelPOPUPReject()" class="btn btn-warning print-btn" id="cancel_temp_po"  data-toggle="modal" data-target="#modal-canceltemPo"  style=" height: 35px; background: #b3b5f;margin-bottom:10px;">Reject</button>	
</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body"> -->
       <div class="table-responsive">
		...
		<table id="tblnotification"	class="table table-bordered st-table"  cellspacing="0" style="width:2500px;">
...
<div id="modal-canceltemPo" class="modal fade" role="dialog">
  ...
  <!-- modal popup for cancel tem Po --> 
	...
	function loadPriceMasterData(childProdId,childProdName){
		...
		priceMasterTableHead+='<th rowspan="2" style="vertical-align:middle">Child Product</th>';
			...
			success : function(data) {
				...
				try{	
					//this code is for printing month names
						...
						if(thirdMonth>0){
							priceMasterTableBody+='<td style="text-align:center;border: 1px solid;">'+data[0][0].child_product_name+'</td>';
							priceMasterTableBody+='<td style="text-align:center;border: 1px solid;">'+data[0][0].measurement_name+'</td>';
						}else if(secondMonth>0){
							priceMasterTableBody+='<td style="text-align:center;border: 1px solid;">'+data[1][0].child_product_name+'</td>';
							priceMasterTableBody+='<td style="text-align:center;border: 1px solid;">'+data[1][0].measurement_name+'</td>';
						}else if(firstMonth>0){
							priceMasterTableBody+='<td style="text-align:center;border: 1px solid;">'+data[2][0].child_product_name+'</td>';
				...
				if (thirdMonth == 0 && secondMonth == 0 && thirdMonth== 0) {
					$("#priceMasterTableBody").html("No Data available from past months");
	...
	function Approve() {
		...
		//	document.getElementById("countOfChargesRows").value = getAllChargesCount();
			document.getElementById("ProductWiseIndentsFormId").action = "UpdatedTempPoApproval.spring";
	...
	function Reject() {
		...
		function myFunction() {
		 ...
		 function openNewWindow() {
			 debugger;
		       return window.open('', '_blank', 'width=800,height=600', 'toolbar=0,location=0,menubar=0');
		       /*  popup.document.write('<html><head><title>Sumadhura-IMS</title><link rel="shortcut icon" type="image/x-icon" href="images/favicon.jpg"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.0/css/bootstrap.min.css"><style>.tbl{border:1px solid #000;}.tbl thead tr th{border:1px solid #000;}#tblnotification thead tr th{background-color:#ccc;border:1px solid #000;text-align:center;}</style></head>'+
						'<body id="printPriceMasterData" style="margin:25px;">'+divToPrint.innerHTML+'</body></html>');  */
		  }
